...
class TestLoginUser:
    ...
    def test_login_user_incorrect_password(self, generate_user_data):
        ...
        with allure.step('Авторизация пользователя с неверным паролем'):
            login_user = UserMethods.login_user_incorrect_password(generate_user_data)
        assert user.status_code == 200
        assert login_user.status_code == 401 and login_user.json()["message"] == MessageAnswer.ERR_INCORRECT_DATA
    ...
    def test_login_user_incorrect_email(self, generate_user_data):
        ...
        with allure.step('Авторизация пользователя с неверным логином'):
            login_user = UserMethods.login_user_incorrect_email(generate_user_data)
        assert user.status_code == 200
        assert login_user.status_code == 401 and login_user.json()["message"] == MessageAnswer.ERR_INCORRECT_DATA
